...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Dark\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=5521968462670:+55 2196846-2670\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '.','!','#'
blocked = []
limitawal = '999999999'
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["5521968462670@s.whatsapp.net"] // replace this with your number
...
break
		   ...
		   await limitAdd(sender)
			  ...
			  case 'next':
                ...
                break
			case 'instaimg':
				if (!isUrl(args[0]) && !args[0].includes(')) return reply(mess.error.lv)
				...
				if (!isUrl(args[0]) && !args[0].includes()) return reply(mess.error.lv)
				    ...
				    insta = getBuffer(anu.result)
				    reply(mess.wait)
				    client.sendMessage(from, insta, video, {mimtype: 'video/mp4', filename: '.mp3, quoted: mek})
				    await limitAdd(sender) 
				    break  
...
// recursos anime, random @dark modz domina vida
          ...
          reply('Espere..')
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT DARK MODZ✨* 」\n*│+ wa.me/5521968462670*╰──────*「 *DARK* 」*────*\n\n*_SE QUER SER ADMIN DO BOT DARK_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				case 'dono':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTu6jWtM0NVS7R9z908dwB4t1wJNTk1RIffEw&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* 𝑫𝑨𝑹𝑲 𝑴𝑶𝑫𝒁 𝑾𝑨⃖ ☔\n*WPP.* wa.me/+5521968462670\n* \nEspero que tenham gostado do bot 🐊🚩️'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
